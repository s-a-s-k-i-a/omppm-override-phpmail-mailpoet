...
Install tooling with `composer install`. Production code targets PHP 8.1+ (the published 8.0 claim is broken and tracked in issue #1) and must stay compatible with the MailPoet-internal classes it overrides. Do not commit `vendor/`, Playground site state, generated ZIP files, or credentials.
